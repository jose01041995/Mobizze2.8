(function() {
    ...
    let apiHistory = [
        { 
            role: "system", 
            content: "És um assistente virtual inteligente da Mobizze, uma agência de IA. És amigável, direto e profissional. Responde sempre em Português de Portugal. O teu objetivo é ajudar o utilizador a entender o poder da automação, mas as respostas devem ser concisas (máximo 2-3 frases curtas)." 
        }
    ];
    ...
    function saveState() {
        localStorage.setItem('mbz_chat_state', JSON.stringify({
            apiHistory,
            uiMessages,
            userMessageCount
        }));
    ...
    restartBtn.addEventListener('click', () => {
        // Limpar storage
        localStorage.removeItem('mbz_chat_state');
        ...
        appendMessage('bot', 'Olá! 👋 Sou o assistente de IA da Mobizze. Pode testar as minhas capacidades agora mesmo. Como o posso ajudar?');
    });
    ...
    function initChat() {
        const saved = localStorage.getItem('mbz_chat_state');
        if (saved) {
    ...
    async function handleSend() {
        ...
        try {
            ...
            if (userMessageCount > 0 && userMessageCount % 3 === 0) {
                setTimeout(() => {
                    appendMessage('system', 'Quer implementar um assistente como este na sua empresa?<br><br>Fale connosco em <a href="contacto.html" style="color: #991b1b; text-decoration: underline; font-weight: bold;">www.mobizze.com/contactos</a>');
                }, 800);
            }
            ...
            removeTyping();
            appendMessage('bot', 'Desculpe, ocorreu um erro de ligação. Tente novamente mais tarde.');
